...
| Assertions can target a specific request path/method outside normal navigation. | Validating AJAX/background/API calls triggered separately from page loads. | `SqlQueryMonitoringRequestMatchingTests.SqlQueryMonitoringShouldAllowAssertingSeparateRequestsByPath` |
...
| LINQ to DB requests are captured and assertable by request path. | Ensuring monitoring is not limited to one ORM/query entry path. | `SqlQueryMonitoringLinqToDbTests.LinqToDbSamplesShouldBeCapturedBySqlMonitoring` |
