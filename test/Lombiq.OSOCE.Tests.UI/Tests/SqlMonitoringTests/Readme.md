...
| Verify follow-up request edge cases. | Page-load and async API requests are captured separately, combined follow-up assertions can work without an explicit page-state wait, old summaries are not pulled into follow-up assertions, and combined follow-up assertions can fail without an explicit request path when duplicates are present. | `SqlQueryMonitoringAsyncRequestScenariosShouldWork` |
