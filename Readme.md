...
- [`Lombiq.HelpfulLibraries.SourceGenerators`](https://github.com/Lombiq/Helpful-Libraries/tree/dev/Lombiq.HelpfulLibraries.SourceGenerators): A collection of helpful source generators. You can watch a demo video of the project [here](https://www.youtube.com/watch?v=MluV1ocZGiE).
- [`Lombiq.OrchardCoreApiClient`](https://github.com/Lombiq/Orchard-Core-API-Client): A client library for communicating with the [Orchard Core](https://www.orchardcore.net/) web APIs, it contains an implementation for the tenant and content management APIs and a console application for testing and demonstration. You can watch a demo video of the project [here](https://www.youtube.com/watch?v=VrYoN34uFQI).
...
- [`Lombiq.Hosting.Tenants.HealthChecks`](https://github.com/Lombiq/Hosting-Tenants/tree/dev/Lombiq.Hosting.Tenants.HealthChecks): A module that makes [Health Checks](https://docs.orchardcore.net/en/main/reference/modules/HealthChecks/) in tenants report back to the Default tenant.
...
- [`Lombiq.LoginAsAnybody`](https://github.com/Lombiq/Orchard-Login-as-Anybody): With the help of this module, an administrator can log in as anybody on the admin dashboard. Watch a demo video of it from the Orchard Community Meeting [here](https://www.youtube.com/watch?v=UlZhblts1BI).
- [`Lombiq.Marketing`](https://github.com/Lombiq/Orchard-Marketing): Shared base project for Orchard Core marketing features. It contains common abstractions for client-side tracking and short URL hit registration. Watch a demo video of it from the Orchard Community Meeting [here](https://www.youtube.com/watch?v=RUDKOZKfXrY).
- [`Lombiq.Marketing.Pirsch`](https://github.com/Lombiq/Orchard-Marketing/tree/dev/Lombiq.Marketing.Pirsch): Basic [Pirsch](https://pirsch.io/ref/BQdZP2jgqM) integration for Orchard Core, with client-side tracking, a first-party script proxy, and server-side tracking for short URL redirects.
- [`Lombiq.Marketing.UrlShortener`](https://github.com/Lombiq/Orchard-Marketing/tree/dev/Lombiq.Marketing.UrlShortener): Adds short URL support to Orchard Core. It can also store UTM values that marketing provider modules can use for server-side tracking during redirects.
...
- [`Lombiq.MSBuild.LibMan.Targets` and `Lombiq.MSBuild.*.Sdk`](https://github.com/Lombiq/MSBuild-Targets/): Contains MSBuild Targets and Props files to streamline repetitive project configuration.
...
- The .NET build workflows of Lombiq GitHub Actions [always default to a specific version under the latest LTS version of the .NET SDK](https://github.com/Lombiq/GitHub-Actions?tab=readme-ov-file#default-net-version) for the same reason. However, the SDK version can be configured if the app being built needs a different one.
